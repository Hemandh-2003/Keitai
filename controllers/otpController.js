...
exports.generateOtp = (email) => {
    ...
    console.log(`Generated OTP for ${email}: ${otp}`);//console
...
exports.validateOtp = (email, otp) => {
    ...
    console.log(`Stored OTP for ${email}: ${otpData ? otpData.otp : 'N/A'}`); // Debug
...
exports.sendOtpEmail = async (email, otp) => {
  ...
  try {
    ...
    console.log(`OTP sent successfully to ${email}:`, info.response); // Debug
    return true;
  } catch (err) {
    console.error('Error sending OTP email:', err); // Debug
    return false;
  }
};
